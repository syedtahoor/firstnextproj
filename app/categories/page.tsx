...
import CategoryAttributesSheet from "@/components/categories/CategoryAttributesSheet";
...
interface Attribute {
  id: number; name: string; placeholder: string;
  sortOrder: number; isVariant: boolean;
...
interface FormState {
  ...
  type: "Product" | "Service"; parent: string; image: File | null;
}

const EMPTY_FORM: FormState = { name: "", sort: "", desc: "", type: "Product", parent: "", image: null };
...
const INITIAL: Category[] = [
  ...
  { id: 5, name: "Cell Phones", sortNumber: null, type: "Service", image: "https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=120&q=70", description: "Smartphones & accessories" },
  { id: 6, name: "Laptops", sortNumber: null, type: "Service", image: "https://images.unsplash.com/photo-1496181133206-80ce9b88a853?w=120&q=70", description: "Portable computers" },
  { id: 7, name: "Jewelry", sortNumber: null, type: "Service", description: "Gold & Silver pieces" },
...
export default function CategoriesPage() {
  ...
  const [categoryAttributes, setCategoryAttributes] = useState<Record<number, Attribute[]>>({});

  const handleSaveAttribute = (attr: Attribute) => {
    if (!activeCategory) return;
    setCategoryAttributes(prev => ({ ...prev, [activeCategory.id]: [...(prev[activeCategory.id] ?? []), attr] }));
  };
  const handleUpdateAttribute = (attr: Attribute) => {
    if (!activeCategory) return;
    setCategoryAttributes(prev => ({ ...prev, [activeCategory.id]: (prev[activeCategory.id] ?? []).map(a => a.id === attr.id ? attr : a) }));
  };
  const handleDeleteAttribute = (id: number) => {
    if (!activeCategory) return;
    setCategoryAttributes(prev => ({ ...prev, [activeCategory.id]: (prev[activeCategory.id] ?? []).filter(a => a.id !== id) }));
  };

  const visible = cats
    .filter(c => c.name.toLowerCase().includes(search.toLowerCase()) && (typeFilter === "All" || c.type === typeFilter))
    .sort((a, b) => {
  ...
  const parentOptions = cats.filter(c => c.type === form.type && c.id !== editId);
  ...
  const openEdit = (c: Category) => { setEditId(c.id); setForm({ name: c.name, sort: c.sortNumber?.toString() ?? "", desc: c.description ?? "", type: c.type, parent: c.parent ?? "", image: null }); setShowForm(true); };
  const handleSave = () => {
    if (!form.name.trim()) return;
    const updated = { name: form.name, sortNumber: form.sort ? Number(form.sort) : null, description: form.desc, type: form.type, parent: form.parent || undefined, image: form.image ? URL.createObjectURL(form.image) : undefined };
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-sm text-gray-700">
      ...
      <CategoriesTable
        ...
        onOpenAttributes={(cat) => { setActiveCategory(cat); setAttrOpen(true); }}
      />
      <CategoriesSheet
        open={showForm} editId={editId} form={form}
        allCategories={parentOptions} onFormChange={setForm}
        onClose={() => setShowForm(false)} onSave={handleSave}
      />
      <CategoryAttributesSheet
        open={attrOpen}
        categoryName={activeCategory?.name ?? null}
        attributes={activeCategory ? (categoryAttributes[activeCategory.id] ?? []) : []}
